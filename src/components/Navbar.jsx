...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { id: '/features', label: 'Features', icon: <FiStar className="mr-1" /> },
    { id: '/pricing', label: 'Pricing', icon: <FiInfo className="mr-1" /> },
              ...
              {navItems.map((item) => (
                <Link
                  ...
                  className={`relative px-3 md:px-4 py-2 text-sm font-medium transition-colors ${
                    isActive(item.id) ? 'text-blue-600' : 'text-gray-600 hover:text-blue-500'
                  }`}
                >
                  ...
                  <div
                    className={`absolute bottom-0 left-0 h-0.5 bg-blue-500 transition-all duration-300 ${
                      hoveredItem === item.id || isActive(item.id) ? 'w-full' : 'w-0'
                    }`}
                  />
                </Link>
              ))}
          ...
          {navItems.map((item) => (
            <Link
              ...
              className={`w-full flex items-center px-3 py-3 rounded-md text-base font-medium ${
                isActive(item.id) ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-blue-50 hover:text-blue-500'
              }`}
            >
              {item.icon}
              {item.label}
